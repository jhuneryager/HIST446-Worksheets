...
>When the negative signs to the equation it does not include the values that are in the parenthesis.
...
> Certain variables are being selected based off of instructions that the operator of the program prompts it with whether that be using the word true to get certain variables or using comparisons to do so.
...
> An operation in the system was applied to give letter descriptions to the number values in the vector.
...
> The difference is the data you get. In the first example you get the data for two of the columns. In the second example you get the data for three rows.
...
BostonWomenVoters[10500:10510,6:7]
```

> I wanted to subset near the bottom of the data set with the precinct and name of the women voters. I chose this because the data set is so large that I wanted to see some data from near the bottom. I thought that seeing the names and precincts would be interesting.
...
> It pulls specific data out of the data sets whether that is large amounts or little amounts.
