...
```{r}
paste("Yay", "Homework is Done")
```

(@) Can you explain what happened in 2-3 sentences? 

> When I used the word paste it connected the two strings together. So now I know that method can be used in the future.
...
5 is not equal to 5
3 is equal to 4 or 3
3 is not equal to 4 and 3 is not equal to 5
...
```{r}
greet_me<-"Hi"
roomie_power<-"Ava and Kaden"
paste(greet_me,roomie_power)
```
...
>We were able to make a variable that housed multiple numbers. Then we were able to multiply all of the numbers by two.
...
>In the code abobe we used the same vector as before. But instead of multiplying all of the numbers by 2 we only multiplied the third number in the set.

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
the_years<-c(2005,2006,2007,2008,2009)
```
...
> It takes a single numeric argument x and returns a numeric vector containing largest integers not greater than the corresponding elements of x
...
> Row z is the results of x and y being put into the formula provided. So the logic is the colum x has (x*2)+y.
...
>The scope is the address book of a man over the course of 20 years. It was also last updated in 2022.
...
> I went back to earlier in the assignment to figure these out. I went to the documentation of head() and found the information I needed.
...
>So what is happening here is gayguides tells the system what data set we are using, $ shows we are looking for a certain segment, title tells it what row we want, and 500 tells it what row we are looking at.
...
head(judges)
str(judges)
head(judges$FirstName)
```

(@) What did you learn about the dataset? What can you tell me about it? 

> I learned information about the federal judges of the US. It has all of the names of judges, the years they were in office, and a bunch of other facts about the judges.
